...
const ModeControls: React.FC = () => {
  ...
  const handleOkClick = () => {
    if (state.selectedAnnotations.length < data.annotation.length) {
      // 未選択の矩形がある場合は警告と強調表示
      alert(`すべての矩形が選択されていません。(${state.selectedAnnotations.length}/${data.annotation.length}個選択済)`);
      // 未選択矩形の強調表示
      flashUnselectedRectangles();
    } else {
      ...
      <div className="action-buttons flex flex-wrap gap-2">
        {state.mode === 'renumber' && (
          <>
            ...
            <button 
              ...
              title="未選択の矩形を強調表示"
            >
              未選択矩形を強調表示 ({data.annotation.length - state.selectedAnnotations.length}個)
            </button>
          </>
        )}
        ...
        {state.mode === 'delete' && (
          <button 
            ...
            disabled={state.selectedAnnotations.length === 0}
          >
            削除 ({state.selectedAnnotations.length}個選択中)
          </button>
        )}
